...
DECLARE c CURSOR FORWARD_ONLY READ_ONLY FAST_FORWARD for
	SELECT DISTINCT TOP 8 CAST(rv.value as int)
	...
	WHERE o.object_id = OBJECT_ID('logparametro') and p.rows > 0 order by 1 desc
OPEN c
...
DECLARE @total_alteracoes BIGINT;
SELECT @total_alteracoes = SUM(qtd_alteracoes)
FROM ##log_carga_logparametro;
...
SELECT CAST(@estimated_total_time_hours_only AS VARCHAR) + ' horas e ' + CAST(@estimated_total_time_minutes AS VARCHAR) + ' minutos' AS EstimatedTotalTime;
