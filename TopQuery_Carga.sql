...
CREATE OR ALTER PROCEDURE sp_CargaTopQuery @DtFim DATE AS
BEGIN
    ...
    DECLARE @DtIni DATE = DATEADD(DAY, -1, @DtFim)
    ...
    BEGIN
        ...
        WITH CombinedStats AS (
            ...
            WHERE dtcoleta >= CONVERT(DATETIME, CONVERT(DATE, '''+convert(varchar, @DtFim, 23)+''')) + ''07:30:00''
    ...
    Where dtcoleta > GETDATE()-30 and (CONVERT(TIME, dtcoleta) >= '07:30:00' AND CONVERT(TIME, dtcoleta) < '20:00:00') and dtcoleta < @DtFim
    ...
    delete from DBA.DBA.DBO.Baseline_Tuning_Consolidado where CAST(dtcoleta as DATE) = @DtFim and Servidor = CAST(@@SERVERNAME AS VARCHAR(100)) 
    ...
    Where dtcoleta > GETDATE()-30 and 
                ((CONVERT(TIME, DTCOLETA) >= '20:00:00' AND CONVERT(TIME, DTCOLETA) <= '23:59:59') OR (CONVERT(TIME, DTCOLETA) >= '00:00:00' AND CONVERT(TIME, DTCOLETA) < '07:30:00')) and dtcoleta < @DtFim
    GROUP BY DATEADD(MINUTE, (DATEDIFF(MINUTE, '20000101', dtcoleta) / 1440)*1440, '20000101')
    ...
    SET 
        ...
        Servidor = CASE WHEN Servidor = 'PANVDBP373' THEN 'PANFDBP373CLA' 
                        ...
                        ELSE Servidor END
    WHERE CAST(dtcoleta AS DATE) = @DtFim
